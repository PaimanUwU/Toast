...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $profileId = "$_SESSION[id]";  // Replace this with the actual profile ID logic
    $postLikes = 0;
    $postDislikes = 0;
    $imagePath = '';
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
        ...
        $query = "INSERT INTO post (Post_Title, Post_Desc, Post_Content, Post_Likes, Post_Dislikes, Post_Image_ID, Profile_ID) VALUES (?, ?, ?, ?, ?, ?, ?)";
        ...
        $query = "UPDATE post SET Post_Image_ID = ? WHERE Post_ID = ?";
        $stmt = mysqli_prepare($connection, $query);
        mysqli_stmt_bind_param($stmt, "si", $postId, $postId);
        mysqli_stmt_execute($stmt);
        $result = mysqli_stmt_get_result($stmt);
    } else {
